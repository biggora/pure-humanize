(function () {
  ...
  function runAllUpdaters() {
    allUpdaters.forEach(function (fn) { fn(); });
  }
  ...
  function triggerCopiedFeedback(btnEl) {
    ...
    const savedNodes = Array.from(btnEl.childNodes).map(function (n) { return n.cloneNode(true); });
    ...
    setTimeout(function () {
      ...
      while (btnEl.firstChild) btnEl.removeChild(btnEl.firstChild);
      savedNodes.forEach(function (node) { btnEl.appendChild(node); });
